...
window.addEventListener('hashchange', (e) => {
    ...
    if (window.location.hash.slice(1)) {
        ...
        const btn = document.querySelector(`.nr${window.location.hash.slice(1)}`);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    getAllCategories();
    getAllGlasses();
    (_a = document.querySelector('h1')) === null || _a === void 0 ? void 0 : _a.addEventListener('click', () => {
        getCocktails();
    });
    ...
    // favorite search
    const listFavoritesBtn = document.querySelector('.list_favorites');
    listFavoritesBtn.addEventListener('click', () => {
        const selectCategory = document.querySelector('#in_category');
        const selectGlass = document.querySelector('#in_glass');
        listFavoritesBtn.classList.toggle('active');
        searchByCategoryGlassFavorite(selectCategory, selectGlass);
    });
    // search by name functionality
    (_b = document.querySelector('#search_btn')) === null || _b === void 0 ? void 0 : _b.addEventListener('click', (e) => {
        e.preventDefault();
        const text = document.querySelector('#in_name');
 ...
 * @param url string
 */
async function getCocktails(url = `https://cocktails.solvro.pl/api/v1/cocktails/?ingredients=1`, page = 1) {
    try {
        let _url1 = url + `&page=${page}`;
        const cocktails = await get_req(_url1);
        NEXT_PAGE = cocktails.meta.currentPage + 1;
        const listOfCocktails = document.querySelector('.cocktails_list');
        ...
        cocktails.data.forEach((cocktail) => {
            ...
            if ((_a = JSON.parse(String(localStorage.getItem('favorite')))) === null || _a === void 0 ? void 0 : _a.includes(cocktail.id)) {
...
async function getAllCategories() {
    try {
        ...
        categories.data.forEach((category) => {
            selectCategory === null || selectCategory === void 0 ? void 0 : selectCategory.insertAdjacentHTML('beforeend', `<option value="${category}">${category}</option>`);
        });
        selectCategory === null || selectCategory === void 0 ? void 0 : selectCategory.addEventListener('change', () => {
            searchByCategoryGlassFavorite(selectCategory, selectGlass);
        });
    }
    catch (error) {
...
async function getAllGlasses() {
    try {
        ...
        const selectGlass = document.querySelector('#in_glass');
        const selectCategory = document.querySelector('#in_category');
        glasses.data.forEach((glass) => {
            selectGlass === null || selectGlass === void 0 ? void 0 : selectGlass.insertAdjacentHTML('beforeend', `<option value="${glass}">${glass}</option>`);
        });
        selectGlass === null || selectGlass === void 0 ? void 0 : selectGlass.addEventListener('change', () => {
            searchByCategoryGlassFavorite(selectCategory, selectGlass);
        });
    }
    catch (error) {
...
async function getCocktail(id) {
    ...
    try {
        ...
        if ((_a = cocktail.data[0]) === null || _a === void 0 ? void 0 : _a.id) {
...
function ChangeView(data) {
    ...
    const instructions = document.querySelector('#main_instructions');
 ...
 */
function searchByCategoryGlassFavorite(selectCategory, selectGlass) {
    const listFavoritesBtn = document.querySelector('.list_favorites');
